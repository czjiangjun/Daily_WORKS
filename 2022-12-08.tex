...
%\node (input1) [io,below of=start] {输入聚类的个数 $k$ 和最大迭代次数 $n$ };
	\node (process1) [process] {电荷密度、势函数的\textrm{FFT}变换};
	\node (process2) [process,below of=process1, xshift= -4cm] {子程序\textrm{GEN\_RC\_GRID}、\textrm{GEN\_SUB\_GRID\_}和\textrm{SET\_RL\_GRID}(文件\textrm{mgrid.f90})};
	\node (process3) [process,below of=process2] {子程序\textrm{INIT\_MPI}确定\textrm{VASP}并行网格通信的初始构造(文件\textrm{main\_mpi.f90})};
	\node (process4) [process,below of=process3] {\textrm{MODULE~mympi}定义\textrm{MPI}的网格通信模式(文件\textrm{mpi.f90})};
	\node (process5) [process,below of=process1, xshift= 4cm] {子程序\textrm{FFT3D\_MPI}(文件\textrm{fftmpi.f90}和\textrm{fftmpiw.f90})};
	\node (process6) [process,below of=process5] {\textrm{FFTW}库函数接口和\textrm{VASP}自带的\textrm{fftlib.f90}};
	\node (process7) [process,below of=process4, xshift=4cm] {通用\textrm{FFTW}库};
...
\draw [arrow] (process1) -- (process5);
\draw [arrow] (process5) -- (process6);
\draw [arrow] (process6) -- (process7);
\draw [arrow] (process2) -- (process5);
