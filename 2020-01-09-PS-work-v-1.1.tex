...
%Introduction
自2001年攻读量子化学专业的研究生以来，我主要从事第一原理材料计算方法和软件研发有关的工作，旁及大型计算环境下的计算软件的运维(主要是编译、安装和稳定化运行)，\textbf{系统掌握了赝势方法、\textrm{FP-LAPW}方法、\textrm{LMTO}方法和\textrm{PAW}方法的程序实现的核心方法、算法及材料计算软件自动流程的代码结构}。现将这些年的软件研发工作简单概括如下:~
\section{优化基组构造算法，提升软件计算效率}
系统掌握\textrm{FP-LAPW}方法， \textbf{通读了\textrm{WIEN2k}全部代码(约71,000行)}，将$\vec k\cdot\vec p$方法引入到\textrm{WIEN2k}软件中，在不损失计算精度的前提下，将计算效率提升了约1$\sim$2个数量级，\textbf{有效提高了软件计算复杂体系的能力}。该相关研究结果，收录在\textit{Intelligent Computing}一书中。%\upcite{kp-method}。%2010-2012年期间，我也曾注意到基于\textrm{WIEN2k}软件发展的\textrm{GW}方法，主要用于含~\textit{d}~和~\textit{f}~电子体系的能带结构计算，但主要是研读代码和理解方法，没有进行大的软件开发工作。

\section{基于物理完善软件功能和性能}
在参与“材料基因工程关键技术与支撑平台”重点专项的课题1的有关研究工作时，\textbf{阅读\textrm{VASP}的核心代码(约110,000行)}，详细分析和梳理\textrm{VASP}中的晶体对称性分析部分的各子程序，\textbf{根据群表示理论，独立编写了代码(约3,000行)，完善\textrm{VASP}软件中晶体的空间群信息的分析与表示功能}。相关软件著作权在申报中。

借鉴\textrm{VASP}中赝芯电荷$\tilde n_c(\vec r)$和补偿电荷$\hat n(\vec r)$的构造方案，改善了开源软件\textrm{AtomPAW}产生的\textrm{PAW}赝势性质。%\upcite{PAW-dataset}。
\textrm{VASP}的赝势构造方案是\textrm{VASP}软件在第一原理计算中保持优势的核心，至今没有开源，\textbf{这一探索未见报道，为研发自主可控的高性能\textrm{PAW}赝势和软件提供了切实可行的技术路线}。

\section{整合高通量材料计算软件，提升计算流程的自动化程度}
完成第一原理材料模拟流程软件\textrm{Material Projects~(MP)~}和多尺度计算流程软件\textrm{Atomic Simulation Environment~(ASE)}的功能模块分析，\textbf{设计了结合\textrm{MP}和\textrm{ASE}长处的数据库支撑和模块加载相结合的复杂计算流程优化设计与控制方案}，可支持一般材料模拟与计算从建模-结构优化-电子结构与物性计算全流程的“一步式”管理，该计算流程应用于本单位和国家超算的高性能计算服务器的材料高通量模拟场景。

%这段经历充实了我对\textrm{Python}模块加载和数据库支持的高通量材料计算流程软件的整体框架和程序实现的知识储备，同时也为高通量材料计算软件深度研发奠定了重要的软件和系统知识。
...
\vskip 15pt
综上所述，我具备较为扎实的材料计算软件开发所必需的算法和代码基础，精通\textrm{PAW}、\textrm{FP-LAPW}的核心算法与软件结构，掌握\textrm{Python}和数据库支持的自动化计算流程软件的代码实现，具备复杂材料计算模拟和相关软件开发所需的研究背景。


%为此我投入了大量的时间和精力，甚至也因此没有能够发表像样的、有分量的研究论文——作为科研工作者考核，这是本人的一个非常严重的不足;~但我之所以仍坚持这一“笨拙”的学习和研究方式，甚至乐此不疲，因为我的实践和坚持证明，
