
%---------------------- TEMPLATE FOR BCC -----------------------------------------------------------%

%%%%%%%%%%%%%%%%%%%%%  % 页眉-页脚设计  %%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{fancyhdr}           %使用页眉-页脚                   %
%\renewcommand{\headrulewidth}{3pt} %页眉(单)线宽(默认黑色)，设为0可以去页眉线
\makeatletter % 双线页眉
\def\headrule{\color{blue}{\if@fancyplain\let\headrulewidth\plainheadrulewidth\fi%
\hrule\@height 0.5pt \@width\headwidth\vskip1pt %上面线为0.5pt粗
\hrule\@height 3.0pt\@width\headwidth  %下面3pt粗
\vskip -2\headrulewidth\vskip -1pt}      %两条线的距离1pt
}
\makeatother

%\renewcommand{\footrulewidth}{3pt} %页脚线宽(默认黑色)，设为0可以去页脚线
\makeatletter % 双线页眉
\def\footrule{{\color{blue}{\if@fancyplain\let\footrulewidth\plainfootrulewidth\fi%
\hrule\@height 3.0pt \@width\headwidth}}
}
\makeatother

\pagestyle{fancy}    %与文献引用超链接style有冲突
%\lhead{\bfseries Result} %页眉左边位置内容，并加粗 
%\lhead{\includegraphics[scale=0.35]{Figures/BCC_logo-1.png}}                                   %在此处插入logo.pdf图片绝对路径 图片靠右
\lhead{\includegraphics[scale=0.35]{~/Documents/Latex_Beamer/Figures/BCC_logo-1.png}}           %在此处插入logo.pdf图片绝对路径 图片靠右
\chead{} % 页眉中间位置内容
\rhead{}
\lfoot{\hei{地址:~~北京市海淀区丰贤中路7号北科产业3号楼 ~~~~ 邮编:~~100094\\
	电话:~~010-59341997/98 ~~~~~~~ 传真:~~010-59341888 ~~~~~~~\!网址:~~\url{www.bcc.ac.cn}}}  %页脚
%\rule{\temptablewidth}{1pt}
%\cfoot{}
%\rfoot{}

%\usepackage{lastpage}
\fancyfoot[C]{} %保留页脚时去掉页码
%\fancyfoot[C]{\fontsize{8.2pt}{6.2pt}\selectfont{\vspace*{-28pt}\textrm{第~\thepage~页/共~\pageref{LastPage}~页}}}      %设置 第x页/共y页 格式
%\fancyfoot[C]{\fontsize{8.2pt}{6.2pt}\selectfont{\vspace*{-28pt}\textrm{No.\thepage of \pageref{Lastpage}}}}      %设置 第x页/共y页 格式

%%%%%%%%%%%%%%%%%  % pagestyleR常用格式  %%%%%%%%%%%%%%%%%%%%%%%%%
%% empty 无页眉页脚
%% plain 无页眉，页脚为居中页码
%% headings 页眉为章节标题，无页脚
%% myheadings 页眉内容可自定义，无页脚
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setlength{\headsep}{50pt}           % 文本距离页眉的距离                      %
%\setlength{\footskip}{30pt}          % 文本距离页脚的距离                      %
