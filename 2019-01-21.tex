...
\centering
\vspace*{-0.10in}
\includegraphics[height=2.55in,width=5.8in,viewport=0 0 1100 455,clip]{Figures/Ewald_method.png}
...
\centering
\vspace*{-0.18in}
\includegraphics[height=3.85in,width=4.2in,viewport=0 0 600 495,clip]{Figures/VASP_Total_ENE.png}
...
%根据定义，逸出功\textrm{(work function)}是一个电子从金属或半导体表面逸出时克服表面势垒所必须做的功。不同金属具有不同的逸出功，即使是同种金属，其逸出功往往与它的晶轴取向和表面状况密切有关。\textrm{Al}的逸出功为$4.2\mathrm{~eV}$。显然这与\textrm{Fermi~}能的值偏差较大，可能的原因:~
%\begin{enumerate}
%	\item \textrm{Kohn-Sham~}方程求解的轨道能级$\varepsilon_i$一般不满足\textrm{Koopmanns~}定理(一个电子的电离势等于它原来占据的轨道能量的相反值)。\\
%		换言之，在低温下，将\textrm{Fermi~}能级的绝对值视电子从费米能级到固体外表面所需要的能量(逸出功)恰恰是\textrm{Koopmanns~}定理的要求
%	\item \textrm{DFT~}是基态理论，它考虑的核心是总能量的密度泛函，严格地说，\textrm{Kohn-Sham~}方程求解的本征值$\varepsilon_i$不具有物理意义，只是在能带理论中，习惯上将本征值近似为轨道能级;~而且因为存在电子自相互作用抵消不尽的问题，除了轨道能级误差较大外，还表现为对能带的带隙低估\\
%		一个典型的极端算例\upcite{PRB37-9919_1988}，在\textrm{B.-O.~}近似下，对于\textrm{H}原子，\textrm{LDA}方法计算的总能为$-0.976~\mathrm{Ry}$(精确值为$-1.0~\mathrm{Ry}$)，但是轨道是能级$-0.54~\mathrm{Ry}$(精确值为$-1.0\textrm{Ry}$)，\textcolor{red}{最高占据能级的误差达6.25\textrm{eV}}
%	\item 式\eqref{eq:V_shift-term}中的$\alpha$和$\beta$的值与赝势和赝电荷密度密切相关
%\end{enumerate}
%因此不能将\textrm{DFT}得到的\textrm{Fermi~}能级与逸出功简单关联。事实上，在\textrm{VASP~}组建议的功函数的方案是:~
%\begin{enumerate}
%	\item 在\textrm{INCAR~}中设置\\
%		\textrm{LVTOT = .TRUE.}\;\; \textcolor{blue}{\# 计算\textrm{work function}(即静电势)}
%	\item 采用\textrm{VASP}组提供的程序\textbf{vtotav.f}处理后，得到真空能级(未扣除\textrm{Fermi~}能级)
%\end{enumerate}
%也主要是基于\textrm{DFT~}的总能计算精确这个思想出发求算功函数，并不是简单求算\textrm{Fermi~}能级即获得逸出功的思路。
