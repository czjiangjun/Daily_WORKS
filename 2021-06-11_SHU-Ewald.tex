\documentclass[11pt, oneside, a4paper]{article}      % Specifies the document class
...
\title{自编\rm{Ewald}能计算软件与\rm{VASP}中的\rm{Ewald}能计算的分别}   %
%\author[a]{Author A}									     %
\author[]{ }   %
%\author[a]{Author B}									     %
...
根据文献\cite{XIE-LU}的(1.7.16)和\cite{R.Martin}的式(F.5)\textrm{Ewald}能计算公式，\textrm{Ewald}能表达式为
\begin{equation}
		\begin{aligned}
			...
			&-\dfrac{\pi}{2\eta^2\Omega}+\dfrac12\sum_{\vec R}\dfrac{\mathrm{erfc}(\eta x)}x\bigg|_{\vec R+\vec r_s-\vec r_s^{\prime}\neq0}-\dfrac{\eta}{\sqrt{\pi}}\delta_{s,s^{\prime}}\bigg\}
		\end{aligned}
	\label{eq:Ewald}
\end{equation}
$\mathrm{erfc}(x)$是误差函数，$\eta$原则上是任意参数。

在\textrm{VASP}软件的源代码中，\textrm{Ewald}能量计算时，式\eqref{eq:Ewald}$Z_s$取为\textcolor{blue}{元素的价电子数}。

以此次计算\ch{NaCl}为例，\ch{Na}的$Z_s=1$，而\ch{Cl}的$Z_s=7$，因此得到的\textrm{Ewald}能量(见附件目录\textrm{VASP\_test/~}中的\textrm{OUTCAR})为~-1248.46682876~\textrm{eV}

在自编\textrm{Ewald}能量计算程序中，\ch{Na+}的$Z_s=1$，而\ch{Cl-}的$Z_s=-1$，因此得到的\textrm{Ewald}能量(见\textrm{OUTCAR})为~-35.69342472603912~\textrm{eV};~事实上，只要将\ch{Cl}的价电子取$Z_s=7$，用自编\textrm{Ewald}能量程序，也将得到\textrm{Ewald}能量为~-1248.454517023087~\textrm{eV}。


补充说明:~\textrm{VASP}和自编计算程序所得\textrm{Ewald}能量剩余部分的差别源自计算中常数的差别。在源代码中，另引入\textrm{Ewald\_3D\_mod.py}代码，将常数取成和\textrm{VASP}中一样，即可计算\textrm{Ewald}能量为~-1248.4668003423121~\textrm{eV}，剩余部分的差别来自误差函数$\mathrm{erfc}(x)$，因为\textrm{VASP}的误差函数$\mathrm{erfc}(x)$是用样条差值得到的。

\textcolor{red}{综上所述，自编\textrm{Ewald}代码本身没有问题，与\textrm{VASP}计算中的差别，在于两者在式\eqref{eq:Ewald}中选取$Z_s$的不同引起的。}
