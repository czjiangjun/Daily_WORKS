...
\subsection{January}
\begin{itemize}
	\item 2021-01-04-08:~\textrm{DVM-DAC}理论
\end{itemize}

\subsection{March}
\begin{itemize}
	\item 2021-03:~\textrm{AtomPAW\_VASP}:~\textrm{rhfatm}嵌入
\end{itemize}

\subsection{April}
\begin{itemize}
	\item 2021-04-06:~清华王崇愚先生合金模型数据处理\textrm{Python-data}
	\item 2021-04-07:~\textrm{VASP}的\textrm{INCAR}参数与说明
\end{itemize}

\subsection{July}
\begin{itemize}
	\item 2021-07-02:~送书名录\\
		刘鹏、施思齐、黄钦、王繁
\end{itemize}

\subsection{August}
\begin{itemize}
	\item 2021-08-16:~钱萍老师委托写部分章节
\end{itemize}

\subsection{October}
\begin{itemize}
	\item 2021-10-17:~东莞参加理论物理所国产软件会议
\end{itemize}

\subsection{December}
\begin{itemize}
	\item 2021-12-18:~课题结题
\end{itemize}

\section{2022}
\subsection{January}
\begin{itemize}
	\item 2022-01:04-07:~\textrm{VASP}赝势实空间和倒空间变换代码分析
	\item 2022-01:10-12:~\textrm{VASP}的\textrm{SOC}代码分析，真正理解\textrm{Euler~Angle}和$\mathbf{l}\cdot\mathbf{s}$的核心含义，角动量算符$\mathbf{l}$的关键是升降算符
	\item 2021-01:15-28:~\textrm{AtomPAW}的\textrm{wavefunction}、\textrm{Potential}与\textrm{VASP}对应部分的量纲换算:~\textrm{AtomPAW}的量纲是\textrm{Ry}和\textrm{a.u.};\textrm{VASP}中的量纲是\textrm{Hartree}和\textrm{Anstrong}
\end{itemize}

\subsection{February}
\begin{itemize}
	\item \item 2022-02:07-11:~\textrm{atomPAW}与\textrm{VASP}的单位统一:~\textrm{atomPAW}的单位是:~\textrm{Ry}和\textrm{a.u.};~\textrm{VASP}的单位是:~\textrm{Hartree}和\textrm{{\AA}}($\mathrm{{\AA}nstr\"om}$)
	\item 2022-02-16:~与北师大龙闰老师组讨论\textrm{SOC}的计算问题，明确“两步”计算策略
	\item 2022-02-17:~\textrm{Intel}公司\textrm{GPU}新产品研讨
	\item 2022-02-18:~中科院物理所(怀柔)吴泉生课题组邀请报告
	\item 2022-02:23:~\textrm{PWmat}公司讨论
	\item 2022-02:21-25:~搭\textrm{Atomat}-\textrm{ASE}服务器
	\item 2022-02:28:~测试全部\textrm{Atomate}和\textrm{FireWorks}
\end{itemize}

\subsection{March}
\begin{itemize}
	\item 2022-03-01:~梳理\textrm{FireWorks}各数据库的关联，掌握\textrm{Python}的类(\textrm{Class})的编写法则
	\item 2022-03-02~与北师大龙闰课题组讨论
	\item 2022-03-03:~编译安装\textrm{CP2k}、\textrm{LAMMPS}
	\item 2022-03-04:~安装\textrm{ABINIT}，注意要用\textrm{MPICH}编译器，然后依次安装\textrm{HDF5}、\textrm{NETCDF-C}、\textrm{NETCDF-FORTRAN}和\textrm{LIBXC}，然后再安装\textrm{ABINIT}
	\item 2022-03-07:~\textrm{Python}制作\textrm{GUI}
	\item 2022-03-08:
\end{itemize}
