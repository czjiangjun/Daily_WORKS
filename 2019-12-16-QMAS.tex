...
\title{基于\rm{PAW}方法计算原子核附近全电子电荷密度}
%----------------------------------------------------------------------------------------------%
...
%Introduction
能量密度方法(\textrm{energy functional method})根据
\section{\rm{PAW}方法的电荷密度计算}
根据\textrm{PAW}方法的波函数表达形式
\begin{equation}
...
\label{PAW_baiseset}
\end{figure}
波函数表示时，通过引入原子波函数$\phi_i(\vec r)$和赝波函数$\tilde\phi+i(\vec r)$考虑了价电子在靠近原子核附近的行为，因此在\textrm{PAW}框架下，将密度算符$|\vec r\rangle\langle\vec r|$代入，可知密度表达式为
\begin{equation}
...
\tilde n^1(\vec r)=&\sum_{n,(i,j)}f_n\langle\tilde\Psi_n|\tilde p_i\rangle\langle\tilde\phi_i|\vec r\rangle\langle\vec r|\tilde\phi_j\rangle\langle\tilde p_j|\tilde\Psi_n\rangle
	\end{aligned}
\end{displaymath}
显然，因为体系波函数$\Psi(r)$可表示原子核附近电子行为，因此在正空间中，由式\eqref{eq:charge}可得到体系的真实电荷密度。\textcolor{blue}{传统的\textrm{PAW}方法，习惯采用平面波基，因此电荷密度也需要在倒空间表示，为了使用较为平缓的平面波来表示电荷，才使用了超软赝势中的补偿电荷(augmented charge)策略，但这一问题在正空间表示中并不存在。}建议基于\textrm{QMAS}理论的计算方案，得到正空间中不同区域的电荷密度表示，进而获得体系精确的电荷密度表达形式。
