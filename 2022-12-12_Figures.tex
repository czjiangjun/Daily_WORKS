...
\begin{tikzpicture}[
    ...
    box-diam/.style={diamond, aspect=2, inner sep=2pt, draw,fill=green!50,node distance=1cm,minimum width=0.5em,text centered,rounded corners,minimum height=2em,thick},
  ...
  \node [box-rect, below=1.0 of Next-time-step, fill=green!30, text width=18.5em, minimum height=0.5em] (Simulation-Verlet) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf{更新每个粒子随时间演化后的位置}:\\$X_i(t_{\mathrm{next}})=X_i(t_{\mathrm{current}})+V_i(t_{\mathrm{current}})\Delta t+\dfrac12a_i(t_{\mathrm{current}})\Delta t^2$\\\textbf{更新每个粒子随时间演化后的受力与加速度}:\\$f_i(t_{\mathrm{next}})=-\nabla U(X_i(t_{\mathrm{next}}))$\\$a_i(t_{\mathrm{next}})=\dfrac{f_i(t_{\mathrm{next}})}{m_i}$\\\textbf{更新每个粒子随时间演化后的速度}\\$V_i(t_{\mathrm{next}})=V_i(t_{\mathrm{current}})+\dfrac12[a_i(t_{\mathrm{current}})+a_i(t_{\mathrm{next}})]\Delta t$}};
%
  \node [box-diam, below=1.0 of Simulation-Verlet, fill=blue!50, text width=12.5em, minimum height=2.5em] (Continue-next-time_step) {\fontsize{9.5pt}{5.5pt}\selectfont{评估模拟体系\\达到平衡态的依据\\达到平衡态的时间步数}};
  \node [box-trape, below=1.0 of Continue-next-time_step, fill=red!50, text width=9.0em, minimum height=0.5em] (Output-equilibrated-system) {\fontsize{9.5pt}{5.5pt}\selectfont{输出达到平衡态时\\描述体系数据}};
...
%
  ...
  \node [box-diam, below=1.0 of Calculate-Stress, fill=blue!50, text width=12.5em, minimum height=2.5em] (Continue-next-stress_step) {\fontsize{9.5pt}{5.5pt}\selectfont{评估模拟体系\\达到指定应力\\达到应力步数上限}};
  \node [box-rect, below=2.5 of Continue-next-stress_step, fill=lime!70, text width=15.0em, minimum height=0.5em] (Young-Modulus) {\fontsize{9.5pt}{5.5pt}\selectfont{计算体系的\textrm{Young}氏模量}};
  \node [box-trape, below=1.0 of Young-Modulus, fill=red!50, text width=9.0em, minimum height=0.5em] (Output-Mechanical-Properties) {\fontsize{9.5pt}{5.5pt}\selectfont{输出达到平衡态时\\描述数据}};
%
%%%%%%% ------------ 定义 虚线方框转角 -----------------  %%%%%%
%\path
%  (Figure.north) ++(-5.0 em, -0.6em) coordinate (Model-fit)
...
%  (Material-Growth.south) ++(0.0em, -1.0em) coordinate (Material-Growth-Cor);
%
\path [arrow, gray,thick] (Simulation-Box.south) -- (Input-Parameter.north);
\path [arrow, gray,thick] (Input-Parameter.south) -- (Modeling-Initial.north);
\path [arrow, gray,thick] (Modeling-Initial.south) -- (Force-Field.north);
%\path [arrow, gray,thick] (User-defineded.west) -- (Modeling-mode.east);
%\path [arrow, gray,thick] (User-defineded.east) -- (Process-Design.west);
%\path [arrow, gray,thick] (Highthrough-1) -- (User-defineded.north);
%\path [arrow, gray,thick] (Highthrough-2) -- (Process-Design.north);
