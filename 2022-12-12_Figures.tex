...
\begin{lrbox}{\mysavebox}%
\begin{tikzpicture}[node distance=1.8cm,
    box-rect/.style={rectangle,draw,fill=darkgray!20,node distance=1cm,text width=12em,text centered,rounded corners,minimum height=2em,thick},
    box-diam/.style={diamond, aspect=1.5, inner sep=2pt, draw,fill=green!50,node distance=1cm,minimum width=0.5em,text centered,rounded corners,minimum height=2em,thick},
    box-trape/.style={trapezium, trapezium left angle = 70, fill=darkgray!50, trapezium right angle=110,minimum width=3cm,minimum height=1cm,text centered,text width =3em,thick},
    arrow/.style={draw,-latex', red},
  ]

  \node [box-rect, fill=magenta!50, text width=10.5em, draw=pink, minimum height=0.5em, thick] at (current page.west)(Simulation-Box) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf 确定模拟体系的结构参数和粒子位置等信息}};
  \node [box-rect, below=1.0 of Simulation-Box, fill=gray!30, text width=18.5em, minimum height=0.5em] (Input-Parameter) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf 确定力学性质模拟所需的参数}};
  \node [box-rect, below=1.0 of Input-Parameter, fill=white, text width=18.5em, minimum height=0.5em, very thin] (Modeling-Initial) {\fontsize{9.5pt}{2.5pt}\selectfont{\textbf{确定粒子初始态的位置}:~$X_i(t=0)$\\\textbf{确定粒子初始态的速度$V_i(t=0)$}}};
  \node [box-rect, below=1.0 of Modeling-Initial, fill=red!30, text width=18.5em, minimum height=0.5em] (Force-Field) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf{确定\textrm{GPUMD}产生的\textrm{NEP}势函数}:~$U_i^{\mathrm{NEP}}(X_i)$\\\textbf{确定模拟的边值条件}:~周期性边界/非周期性边界\\\textbf{确定模拟的时间步长}:~$\Delta t$}};
  \node [box-rect, below=1.0 of Force-Field, fill=red!30, text width=22.5em, minimum height=0.5em] (Calculation-Force) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf{计算每个粒子在当前时点的受力和加速度}:\\$f_i(t_{\mathrm{current}})=-\nabla U(X_i(t_{\mathrm{current}}))~\mbox{且}~a_i(t_{\mathrm{current}})=\dfrac{f_i(t_{\mathrm{current}})}{m_i}$}};
  \node [box-rect, below=1.0 of Calculation-Force, fill=brown!30, text width=12.5em, minimum height=0.5em] (Next-time-step) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf{移动到下一时间步}:~\\$t_{\mathrm{text}}=t_{\mathrm{current}}+\Delta t$}};
  \node [box-rect, below=1.0 of Next-time-step, fill=green!30, text width=21.5em, minimum height=0.5em] (Simulation-Verlet) {\fontsize{9.5pt}{5.5pt}\selectfont{\textbf{更新每个粒子随时间演化后的位置}:\\$X_i(t_{\mathrm{next}})=X_i(t_{\mathrm{current}})+V_i(t_{\mathrm{current}})\Delta t+\dfrac12a_i(t_{\mathrm{current}})\Delta t^2$\\\textbf{更新每个粒子随时间演化后的受力与加速度}:\\$f_i(t_{\mathrm{next}})=-\nabla U(X_i(t_{\mathrm{next}}))$\\$a_i(t_{\mathrm{next}})=\dfrac{f_i(t_{\mathrm{next}})}{m_i}$\\\textbf{更新每个粒子随时间演化后的速度}\\$V_i(t_{\mathrm{next}})=V_i(t_{\mathrm{current}})+\dfrac12[a_i(t_{\mathrm{current}})+a_i(t_{\mathrm{next}})]\Delta t$}};
%
  \node [box-diam, below=1.0 of Simulation-Verlet, fill=blue!50, text width=12.5em, minimum height=2.5em] (Continue-next-time-step) {\fontsize{9.5pt}{5.5pt}\selectfont{评估模拟体系\\达到平衡态的依据\\达到平衡态的时间步数}};
  \node [box-trape, below=1.0 of Continue-next-time-step, fill=red!50, text width=9.0em, minimum height=0.5em] (Output-equilibrated-system) {\fontsize{9.5pt}{5.5pt}\selectfont{输出达到平衡态时\\描述体系数据}};

  \node [box-rect, right=1.8 of Output-equilibrated-system, fill=orange!50, text width=15.0em, minimum height=0.5em] (Canonical-ensemble) {\fontsize{9.5pt}{5.5pt}\selectfont{分子动力学\\\textrm{NVT/NPT}系综模拟}};
  \node [box-rect, below=0.8 of Canonical-ensemble, fill=white, text width=15.0em, minimum height=0.5em, very thin] (Range-Material-grow) {\fontsize{8.5pt}{5.5pt}\selectfont{构建材料生长模块尺寸与范围}};
  \node [box-rect, below=1.0 of Range-Material-grow, fill=blue!30, text width=15.0em, minimum height=0.5em] (Atom-deposition) {\fontsize{9.5pt}{5.5pt}\selectfont{在指定位置安放原子}};
  \node [box-rect, below=0.5 of Atom-deposition, fill=blue!30, text width=15.0em, minimum height=0.5em] (Atom-Stress) {\fontsize{9.5pt}{5.5pt}\selectfont{计算安放原子后的体系能量和原子受力}};
%  \node [box-rect, below=1.0 of Atom-Stress, fill=blue!30, text width=15.0em, minimum height=0.5em] (Atom-Move) {\fontsize{9.5pt}{5.5pt}\selectfont{将每个原子\\在应力方向上按步长移动}};
  \node [box-rect, below=1.0 of Atom-Stress, text width=15.0em, minimum height=0.5em] (Optimization) {\fontsize{9.5pt}{5.5pt}\selectfont{将模拟体系中每个原子\\在应力方向上按步长移动}};
%
  \node [box-rect, below=0.8 of Optimization, fill=purple!30, text width=15.0em, minimum height=0.5em] (Continue-next-Energy-step) {\fontsize{9.5pt}{5.5pt}\selectfont{充分弛豫，能量最小化}};
  \node [box-rect, below=0.8 of Continue-next-Energy-step, fill=green!30, text width=15.0em, minimum height=0.5em] (Atom-Move) {\fontsize{9.5pt}{5.5pt}\selectfont{计算体系中的每个原子\\的受力和应力}};
  \node [box-diam, below=0.8 of Atom-Move, fill=blue!50, text width=10.0em, minimum height=0.5em] (Calculate-Stress) {\fontsize{9.5pt}{5.5pt}\selectfont{评估增加沉降原子后体系是否稳定}};
  \node [box-rect, below=0.8 of Calculate-Stress, fill=lime!70, text width=15.0em, minimum height=0.5em] (Young-Modulus) {\fontsize{9.5pt}{5.5pt}\selectfont{计算体系的\textrm{Young}氏模量}};
  \node [box-diam, below=0.8 of Young-Modulus, fill=blue!50, text width=12.5em, minimum height=2.5em] (Continue-next-stress-step) {\fontsize{9.5pt}{5.5pt}\selectfont{评估模拟体系\\生长模块尺寸\\是否到达指定范围}};
  \node [box-rect, below=0.8 of Continue-next-stress-step, fill=olive!60, text width=15.0em, minimum height=0.5em, very thin] (NPT-Canonical-ensemble) {\fontsize{9.5pt}{5.5pt}\selectfont{分子动力学\\\textrm{NVT/NPT}系统模拟}};
  \node [box-trape, below=0.8 of NPT-Canonical-ensemble, fill=red!50, text width=9.0em, minimum height=0.5em] (Output-Mechanical-Properties) {\fontsize{9.5pt}{5.5pt}\selectfont{输出达到平衡态时\\描述力学性质数据}};
%  \node [box-rect, below=0.8 of Continue-next-stress-step, fill=lime!70, text width=15.0em, minimum height=0.5em] (Young-Modulus) {\fontsize{9.5pt}{5.5pt}\selectfont{计算体系的\textrm{Young}氏模量}};
%
%%%%%%% ------------ 定义 虚线方框转角 -----------------  %%%%%%
\path
  (Simulation-Verlet.west) ++(-1.5 em, -0.0em) coordinate (Continue-1)
  (Optimization.west) ++(-1.5em, 0.0em) coordinate (Continue-2)
  (Optimization.east) ++(1.0em, 0.0em) coordinate (Continue-3)
  (Range-Material-grow.south) ++(0.0em, -1.0em) coordinate (Corner-1)
  (Optimization.north) ++(0.0em, 1.0em) coordinate (Corner-2)
  (Force-Field.east) ++(3.5em, 4.0em) coordinate (Return)
  
  (Atom-deposition.north west) ++(-0.5em, 0.5em) coordinate (Corner-Atom-1) 
  (Calculate-Stress.south east) ++(6.0em, -2.6em) coordinate (Corner-Atom-2) 

  (Range-Material-grow.north west) ++(-2.5em, 0.5em) coordinate (Corner-Material-1) 
  (Continue-next-stress-step.south east) ++(6.0em, -3.3em) coordinate (Corner-Material-2) 
  ;
%
\path [arrow, black,thick] (Simulation-Box.south) -- (Input-Parameter.north);
\path [arrow, black,thick] (Input-Parameter.south) -- (Modeling-Initial.north);
\path [arrow, black,thick] (Modeling-Initial.south) -- (Force-Field.north);
\path [arrow, black,thick] (Force-Field.south) -- (Calculation-Force.north);
\path [arrow, black,thick] (Calculation-Force.south) -- (Next-time-step.north);
\path [arrow, black,thick] (Next-time-step.south) -- (Simulation-Verlet.north);
\path [arrow, black,thick] (Simulation-Verlet.south) -- (Continue-next-time-step.north);
\path [arrow, black,thick] (Continue-next-time-step.south) -- node[midway,outer sep=0pt, xshift=1.0em, yshift=0.8em](TextNode1){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(Output-equilibrated-system.north);
%
\path [arrow, black,thick] (Canonical-ensemble.south) -- (Range-Material-grow.north);
\path [arrow, black,thick] (Range-Material-grow.south) -- (Atom-deposition);
\path [arrow, black,thick] (Atom-deposition) -- (Atom-Stress);
\path [arrow, black,thick] (Atom-Stress) -- (Optimization.north);
\path [arrow, black,thick] (Optimization.south) -- (Continue-next-Energy-step.north);
\path [arrow, black,thick] (Continue-next-Energy-step.south) -- (Atom-Move.north);
\path [arrow, black,thick] (Atom-Move.south) -- (Calculate-Stress.north);
\path [arrow, black,thick] (Calculate-Stress.south) -- node[midway,outer sep=0pt, xshift=-0.5em](TextNode4){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(Young-Modulus.north);
\path [arrow, black,thick] (Young-Modulus.south) -- (Continue-next-stress-step.north);
\path [arrow, black,thick] (Continue-next-stress-step.south) -- node[midway,outer sep=0pt, xshift=-1.0em](TextNode4){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(NPT-Canonical-ensemble.north);
\path [arrow, black,thick] (NPT-Canonical-ensemble.south) -- node[midway,outer sep=0pt, xshift=-1.0em](TextNode4){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(Output-Mechanical-Properties.north);
%
\path [arrow, black,thick] (Continue-next-time-step.west) node[midway,below=0.05,outer sep=0pt, xshift=-12.0em](TextNode2){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{否}}}}-| (Continue-1) |- (Calculation-Force.west);
\path [arrow, black,thick] (Continue-next-time-step.south)++(0.0em, -1.2em) -| (Canonical-ensemble.north);

\draw [black, thick] (Continue-next-stress-step.west) -| node[midway,below=0.05,outer sep=0pt, xshift=-0.6em, yshift=20.0em ](TextNode3){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{否}}}} (Continue-2) |- (Corner-1) ;
\draw [black, thick] (Calculate-Stress.east) -| node[midway,below=0.05,outer sep=0pt, xshift=-0.6em, yshift=10.0em ](TextNode3){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{否}}}} (Continue-3) |- (Corner-2);

\node [rectangle,draw,dashdotted,inner sep=0.1em,fit=(Corner-Atom-1) (Corner-Atom-2)] (enclosure) {};
\node [rectangle,draw,dotted,inner sep=0.1em,fit=(Corner-Material-1) (Corner-Material-2)] (enclosure) {};
%
%%%%%%% ------------ 绘制 虚线 -----------------  %%%%%%
...
%\label{Fig:GPUMD_Mechanical-Stacking}
%\end{figure}
...
\section{化学反应方程式的书写}
\ce{2H2+O2 ->[\textrm{燃烧}][$\Delta$] 2H2O} %%催化剂
\ce{2H2+O2 $\xlongequal[\triangle]{\ce{MnO2}}$ 2H2O} % 一个简单的化学公式
\ce{2H2+O2  ->[\textrm{燃烧}][$\triangle$] 2H2O} % 一个简单的化学公式
\ce{#} %% 三键
\ce{->} %%箭头
\ce{<=>}  %%可逆号
\ce{<=>[\textrm{催化剂}][$\triangle$]}
\ce{<-->[\textrm{放电}][\textrm{充电}]} %%双向箭头
\ce{<=>>[\textrm{催化剂}][$\triangle$]} %%长短箭头符号
\ce{<<=>[\textrm{催化剂}][$\triangle$]}
...
%  \node [rectangle,draw=red, fill=gray!30, inner sep=0.1em,fit=(Fock-matrix-fit) (Kohn-Sham-fit), thick] (enclosure1) {};
 ...
 % \path [arrow] (OS) -- (Figure);
  ...
  \path [draw,magenta] (Criterion.west)++(0.25em, 0.0em)--(Criterion-fit) node[midway,above=-0.05,outer sep=0pt](TextNode2){\fontsize{4.0pt}{2.5pt}\selectfont{\textrm{\textcolor{black}{NO}}}};
  ...
  \node [box, fill=magenta!50, text width=8.5em, draw=pink, minimum height=0.5em, thick] at (-3.8,9.15) (Figure) {\includegraphics[width=1.05in, height=0.7in,viewport=0 0 1172 863,clip]{Figures/DFT.png}};
...
\begin{tikzpicture}[
  ...
  \node [box-rect, fill=magenta!50, text width=8.5em, draw=pink, minimum height=0.5em, thick] at (current page.west)(Figure) {\includegraphics[width=1.25in, height=0.7in,viewport=0 0 103 55,clip]{Figures/The-Periodic-Table-of-perovskene-materials.jpg}};
...
\begin{tikzpicture}[
  ...
  \node [box-rect, below=0.8 of DataBase, fill=olive!60, text width=12.5em, minimum height=2.5em] (Data-Manage) {\fontsize{7.5pt}{5.5pt}\selectfont{计算结果数据与管理}};
  \node [box-rect, below=0.8 of Data-Manage, fill=purple!30, text width=6.5em, minimum height=1.5em,xshift=-3.5em] (Mechanism-Parameter) {\fontsize{7.5pt}{2.5pt}\selectfont{指定不同规模的粒子体系的力学性质控制参数}};
  \node [box-rect, right=0.5 of Mechanism-Parameter, fill=teal!70, text width=15.5em, minimum height=1.5em] (Machine-Learning) {\fontsize{7.5pt}{5.5pt}\selectfont{\textrm{Python}集成的机器学习算法\textrm{GPR/BO/ANN}支持}};
  \node [box-rect, below=2.5 of Data-Manage, fill=darkgray!70, text width=15.5em, minimum height=1.5em, xshift=5.0em] (Application) {\fontsize{7.5pt}{5.5pt}\selectfont{\textcolor{white}{特色应用场景:~精确的原子间多体相互作用}}};
  \node [box-rect, below=0.05 of Multi-Scale-Simulation, fill=white, text width=0.5em, minimum height=11.0em, very thin] (Result-Machine-Learning) {\fontsize{7.5pt}{2.5pt}\selectfont{计算结果分析与机器学习\vskip 0.4pt }};

  \node [box-rect, below=0.5 of Application, fill=gray!30, text width=12.5em, minimum height=1.5em] (GPUMD-Mechanism) {\fontsize{7.5pt}{2.5pt}\selectfont{\textrm{GPUMD}的基本力学性质计算}};
  \node [box-rect, below=0.8 of GPUMD-Mechanism, fill=lime!70, text width=6.5em, minimum height=1.5em] (Result-Analysis) {\fontsize{7.5pt}{5.5pt}\selectfont{计算结果可视化分析}};
...
\path
  ...
  (Machine-Learning.north) ++(-7.5em, -0.5em) coordinate (Machine-Learning-1)
  (Machine-Learning.south) ++(7.5em, 0.5em) coordinate (Machine-Learning-2)
  (Machine-Learning.east) ++(1.0em, 0.0em) coordinate (Machine-Learning-3);
...
\path [arrow, draw=black, thick](Data-Manage.south)++(-3.45em, 0.0em) -- (Mechanism-Parameter.north);
\path [arrow, draw=black, thick](Mechanism-Parameter.south) |- (GPUMD-Mechanism.west);
\path [arrow, draw=black, thick](Data-Manage.south)++(0.65em, 0.0em) -- (Application-2);
\path [arrow, draw=black, thick](Machine-Learning.north) |- (Data-Manage.east);
\path [arrow, draw=black, thick](Machine-Learning.south) -- (Application-3);
\path [arrow, draw=black, thick](Machine-Learning.east) -- (Machine-Learning-3) |- (GPUMD-Mechanism.east);
\path [arrow, draw=black, thick](Application.south) -- (GPUMD-Mechanism.north);
\path [arrow, draw=black, thick](GPUMD-Mechanism.south) -- (Result-Analysis.north);
...
\end{tikzpicture}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Fig:GPUMD-Atoms-Materials-1}
\end{figure}
...
\begin{tikzpicture}[
  ...
  \node [box-rect, below=0.5 of Application, fill=gray!30, text width=12.5em, minimum height=1.5em] (GPUMD-Mechanism) {\fontsize{7.5pt}{2.5pt}\selectfont{\textrm{GPUMD}的基本力学性质计算}};
  \node [box-rect, below=3.8 of Mechanism-Parameter, fill=teal!30, text width=6.5em, minimum height=1.5em] (Atom-Settlement) {\fontsize{7.5pt}{2.5pt}\selectfont{原子沉积过程的\textrm{GPUMD}模拟}};
  \node [box-rect, right=0.5 of Atom-Settlement, fill=cyan!30, text width=6.5em, minimum height=1.5em] (AI-method) {\fontsize{7.5pt}{2.5pt}\selectfont{基于人工智能算法\textrm{GB/GNN/RF}的原子沉积模式选择}};
  \node [box-rect, right=0.5 of AI-method, fill=violet!30, text width=6.5em, minimum height=1.5em] (Material-Growth) {\fontsize{7.5pt}{2.5pt}\selectfont{材料生长过程的\textrm{GPUMD}模拟}};
  \node [box-rect, below=0.5 of AI-method, fill=lime!70, text width=6.5em, minimum height=1.5em] (Result-Analysis) {\fontsize{7.5pt}{5.5pt}\selectfont{计算结果可视化分析}};
...
\path
  ...
  (Machine-Learning.south) ++(7.5em, 0.5em) coordinate (Machine-Learning-2)
  (Machine-Learning.east) ++(1.0em, 0.0em) coordinate (Machine-Learning-3)
  (GPUMD-Mechanism.south) ++(0.0em, -1.0em) coordinate (GPUMD-Mechanism-Div)
  (Atom-Settlement.south) ++(0.0em, -1.0em) coordinate (Atom-Settlement-Cor)
  (Material-Growth.south) ++(0.0em, -1.0em) coordinate (Material-Growth-Cor);
...
\path [arrow, draw=black, thick](Machine-Learning.east) -- (Machine-Learning-3) |- (GPUMD-Mechanism.east);
\path [arrow, draw=black, thick](Application.south) -- (GPUMD-Mechanism.north);
\path [arrow, draw=black, thick](GPUMD-Mechanism.south) -- (GPUMD-Mechanism-Div) -| (Atom-Settlement.north);
\path [arrow, draw=black, thick](GPUMD-Mechanism.south) -- (GPUMD-Mechanism-Div) -| (Material-Growth.north);
\path [arrow, dotted,  draw=black, thick](Atom-Settlement.east) -- (AI-method.west);
\path [arrow, dotted,  draw=black, thick](AI-method.east) -- (Material-Growth.west);
\path [arrow, draw=black, thick](Atom-Settlement.south) -- (Atom-Settlement-Cor) |- (Result-Analysis.west);
\path [arrow, draw=black, thick](Material-Growth.south) -- (Material-Growth-Cor) |- (Result-Analysis.east);
\draw [dash dot dot, black, thick](GPUMD-Mechanism.south) -- (AI-method.north);
\path [arrow, dash dot dot, draw=black, thick](AI-method.south) -- (Result-Analysis.north);
...
\end{tikzpicture}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Fig:GPUMD-Atoms-Materials-2}
\end{figure}
...
\end{tikzpicture}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Fig:GPUMD-Atoms-Materials-3}
\end{figure}
...
%
  ...
  \node [box-rect, right=2.0 of Force-Field, fill=orange!50, text width=15.0em, minimum height=0.5em] (Canonical-ensemble) {\fontsize{9.5pt}{5.5pt}\selectfont{分子动力学\\\textrm{NVT/NPT}系综模拟}};
  \node [box-rect, below=0.8 of Canonical-ensemble, fill=white, text width=15.0em, minimum height=0.5em, very thin] (Small-Strain) {\fontsize{8.5pt}{5.5pt}\selectfont{在体系一个方向上施加应力步长}};
  \node [box-rect, below=0.8 of Small-Strain, text width=15.0em, minimum height=0.5em] (Optimization) {\fontsize{9.5pt}{5.5pt}\selectfont{将模拟体系在应力作用方向充分弛豫，能量最小化}};
%
  \node [box-rect, below=0.8 of Optimization, fill=olive!60, text width=15.0em, minimum height=0.5em, very thin] (NPT-Canonical-ensemble) {\fontsize{9.5pt}{5.5pt}\selectfont{\textrm{NPT}系统模拟}};
  \node [box-rect, below=0.8 of NPT-Canonical-ensemble, fill=purple!30, text width=15.0em, minimum height=0.5em] (Calculate-Stress) {\fontsize{9.5pt}{5.5pt}\selectfont{计算下一应力步长条件下的状态}};
  \node [box-diam, below=0.8 of Calculate-Stress, fill=blue!50, text width=12.5em, minimum height=2.5em] (Continue-next-stress-step) {\fontsize{9.5pt}{5.5pt}\selectfont{评估模拟体系\\达到指定应力\\达到应力步数上限}};
  \node [box-trape, below=0.8 of Continue-next-stress-step, fill=red!50, text width=9.0em, minimum height=0.5em] (Output-Mechanical-Properties) {\fontsize{9.5pt}{5.5pt}\selectfont{输出达到平衡态时\\描述力学性质数据}};
  \node [box-rect, below=0.8 of Output-Mechanical-Properties, fill=lime!70, text width=15.0em, minimum height=0.5em] (Young-Modulus) {\fontsize{9.5pt}{5.5pt}\selectfont{计算体系的\textrm{Young}氏模量并输出}};
%
%%%%%%% ------------ 定义 虚线方框转角 -----------------  %%%%%%
\path
  (Simulation-Verlet.west) ++(-1.5 em, -0.0em) coordinate (Continue-1)
  (NPT-Canonical-ensemble.east) ++(1.5em, 0.0em) coordinate (Continue-2)
  (Force-Field.east) ++(3.5em, 4.0em) coordinate (Return)
  ;
%
\path [arrow, black,thick] (Simulation-Box.south) -- (Input-Parameter.north);
...
\path [arrow, black,thick] (Next-time-step.south) -- (Simulation-Verlet.north);
\path [arrow, black,thick] (Simulation-Verlet.south) -- (Continue-next-time-step.north);
\path [arrow, black,thick] (Continue-next-time-step.south) -- node[midway,outer sep=0pt, xshift=-1.0em](TextNode1){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(Output-equilibrated-system.north);
%
\path [arrow, black,thick] (Canonical-ensemble.south) -- (Small-Strain.north);
\path [arrow, black,thick] (Small-Strain.south) -- (Optimization.north);
\path [arrow, black,thick] (Optimization.south) -- (NPT-Canonical-ensemble.north);
\path [arrow, black,thick] (NPT-Canonical-ensemble.south) -- (Calculate-Stress.north);
\path [arrow, black,thick] (Calculate-Stress.south) -- (Continue-next-stress-step.north);
\path [arrow, black,thick] (Continue-next-stress-step.south) -- node[midway,outer sep=0pt, xshift=-1.0em](TextNode4){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{是}}}}(Output-Mechanical-Properties.north);
\path [arrow, black,thick]  (Output-Mechanical-Properties.south)-- (Young-Modulus.north);
%
\path [arrow, black,thick] (Continue-next-time-step.west) node[midway,below=0.05,outer sep=0pt, xshift=-12.0em](TextNode2){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{否}}}}-| (Continue-1) |- (Calculation-Force.west);
\path [arrow, black,thick] (Continue-next-time-step.south)++(0.0em, -1.2em) -| (Return) -| (Canonical-ensemble.north);
\path [arrow, black,thick] (Continue-next-stress-step.east) node[midway,below=0.05,outer sep=0pt, xshift=31.0em, yshift=8.0em](TextNode3){\fontsize{8.0pt}{4.5pt}\selectfont{\textbf{\textcolor{black}{否}}}}-| (Continue-2) |- (Small-Strain.east);
%
%%%%%%% ------------ 绘制 虚线 -----------------  %%%%%%
%%\draw [gray, dotted, line width=2pt] (Symmetry-seperate) -- (Modeling-Initial.south);
%\draw [gray, dashed, ultra thick] (Symmetry-seperate) -- (Modeling-Initial.south);
...
\end{tikzpicture}
%\caption{\tiny \textrm{Pseudopotential for metallic sodium, based on the empty core model and screened by the Thomas-Fermi dielectric function.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Fig:GPUMD_Mechanical-Properties}
\end{figure}
...
%    arrow/.style={draw,-latex', line width = 2pt},
%		]
%		\node [box-rect, text width=2.0em, minimum height=1.5em, yshift=5.0em, very thin] (Melody) {\fontsize{8.5pt}{2.5pt}\selectfont{唱腔}}; % yshift=5.0em 表示竖直方向，向上移动3.5em
...
%
%\draw [dotted, very thick](-8, -10.0) -- (-8, 2.5);
%\draw [dotted, very thick](8, -10.0) -- (8, 2.5);
