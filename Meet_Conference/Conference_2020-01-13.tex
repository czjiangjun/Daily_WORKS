...
\section{杨明理报告}
材料高通量计算:~数据生产者
\subsection{材料高通量计算的基本内容}
\textrm{High-Throughput}
利用单次实验获得多个样品的信息

\textrm{HTC}:~面对计算密集型任务\\
\begin{itemize}
	\item 核心:工作流(\textrm{Workflow})
	\item \textrm{Queuing system}
\end{itemize}

\subsection{材料计算为什么需要高通量}
\begin{itemize}
	\item 多组元材料体系
	\item 复杂势能米面
	\item 多工艺流程
...
\begin{itemize}
	...
	\item 计算结果的可比性
	\item 计算结果的分析
	\item 确定筛选因子:~物理模型、物理概念、机器学习
	\item 成本控制:~计算资源占用量
\end{itemize}

\subsection{材料基因工程:~高通量计算}
\textrm{idea}$\rightarrow$\textrm{result}

\textrm{Material Projects}、\textrm{AFLOW}、\textrm{NOMD}、等

\subsection{材料高通量平台建设}
国内几个大超算

\subsection{高通量计算应用-热点}
\begin{itemize}
\item {高通量计算应用-计算方法}:~ \textrm{DFT}应用广泛、局限性
\item {高通量计算应用-数据}
\item {高通量计算应用-高通量实验}
\item {高通量计算应用-预测材料结构和性质}
\item {高通量计算应用-预测材料合成路径}
\item 川大:~\textrm{Biomaterial Finder}
	\begin{itemize}
		\item 工作流范式设计:~设计过程
		\item 工作流规范设计
	\end{itemize}
...
\subsection{先验知识机器学习势函数}
减少特征变量的算法

\subsection{基于机器学习的跨尺度计算}
机器学习势函数:~分子动力学-相场计算无缝衔接

\subsection{高通量多尺度平台}
正向预测与反向设计算法提高$\mathrm{UO}_2$的热导率:~针对热导率单一目标优化
\textrm{TRISO}颗粒和\textrm{FCM}燃料有限元模拟

\subsection{数据方法:~适应性设计}
考虑误差:~利用\textrm{Gaussian}过程考虑误差
设计算法:\textrm{EGO}.~\textrm{vs}.~\textrm{KG}

\section{陈星秋报告}
\textrm{Atomistic Modeling and Algorithm}
\subsection{Challenging of simulations}
\begin{itemize}
	\item \textrm{Electrochemical corrosion}
	\item \textrm{Modeling}:~\textrm{Modeling}+\textrm{Algorithm}\\
		\textrm{Surfaces or interfaces of each half-cell}
	\item \textrm{Why does Mg exhibit weak corrosion}
\end{itemize}

\section{王剑秋报告}
\textrm{Material selection and corrosion Behavior in Nuclear Power Plants}
\subsection{Background}
化石能源依赖过高，核能发展快速。压水堆为主\\
蒸汽发生器的选材\\
开裂问题

\section{高克玮报告}
环境失效过程的高通量计算模拟与高通量实验技术
\subsection{研究背景}
材料服役行为的评价
