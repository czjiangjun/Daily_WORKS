...
%\label{eq:magno-1}
%\end{equation}
\section{高通量数据的计算模拟解析}
上海大学~鲁晓刚~教授
\begin{itemize}
	\item 高通量实验:~(1)~扩散多元节:~扩散偶技术\\
		(2)组合材料芯片
	\item 高通量计算:~(2)单作业高度并行\\
		(2)多作业并发式运作\\
		(3)风险与问题\\
		\textrm{TCP}~Phase(聚相)/无序固溶体相
	\item 计算解析数据:~问题(1)扩散数据不能直接使用;~(2)实验数据离散;~(3)不能充分利用二维扩散系数\\
		方法:~\textrm{CALPHAD}:~(扩散热力学、扩散动力学):~计算扩散系数$\rightarrow$原子活动性参数(温度、成分的关系)\\
		扩散偶技术:~提取相图及扩散系数\\
		\textrm{CALPHAD}~多元数据库:根据二元、三元模拟出多元数据库\\
		(4)实验数据离散、矛盾\\
		方法:~\textrm{CALPHAD}+热物理模型:~计算热力学、状态方程(热容、体积、热膨胀、弹性模量、高压性质)\\
		(5)数据量过大
		方法:~大数据的挖掘
	\item 材料数据库~(1)材料设计基础数据库:~模型参数$\rightarrow$多元材料
		(2)材料实验数据库(大数据):
\end{itemize}
问题:~含杂质的多元系能否应用该问题\\
答:~外推简化
	
\section{基于大数据和\rm{AI}的药物设计}
\begin{itemize}
	\item 人工智能药物研发项目与人工智能药物发现的概念
	\item 研究方向和进展
		\begin{itemize}
			\item 重点研究方向:~(1)药物多靶点活性预测:~单靶点设计$\rightarrow$多靶点应用\\
				(2)基于多任务深度神经网络的药物激酶谱预测\\
				(3)成药性扩展空间数据库:~\textrm{DrugspaceX}
			\item 药物分子结构的表示学习\\
				药物设计:~基于分子的联想
			\item 药物作用机制和靶标的发现:~作用机制和靶标发现是药物研究的难点\\
				类似于人脸识别:~化合物结构特征
		\end{itemize}
	\item 存在问题与展望
\end{itemizE}

\section{太阳能电池能源转化机制的第一原理研究}
\begin{itemize}
	\item 研究背景
		石化能源，不可再生\\
		燃料敏化太阳能电池:~\ch{TiO2}:~载流子动力学过程\\
		成本低，效率低\\
		加速电子注入过程
		钙钛矿太阳能点成~\ch{ABM}\\
		成本更低，吸光性好，稳定性差
	\item 计算模拟\\
		激发态电子动力学模拟:~\textrm{TDAP}和\textrm{Trajectory~Surface~Hopping}\\
		界面电子注入和复合过程的模拟
		电池的短路电流模拟计算
\end{itemize}<++>
