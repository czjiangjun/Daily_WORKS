...
%\label{eq:magno-1}
%\end{equation}
简介:~晶体材料国家重点实验室和科研究团队

\section{研究思想}
晶体结构\textrm{vs}电子结构:~理化性质
...
\section{报告主要研究}
\begin{itemize}
	\item 光催化过程:~光吸收、载流子分离(迁移和复合)、表面/界面氧化-还原反应
	\item 宽禁带半导体
	\item 表面等离子体光催化及能量转移机理
	\item 光解水及能源、环境材料的设计、结构性质和性能
	\item 合成氨与单原子催化:~金属单原子/非金属单原子\textrm{@g-CN}体系固氮的高活性和选择性
	\item 目前研究的工作
\end{itemize}

\begin{itemize}
	\item \textrm{Topological crystalline/ Chern insulators}
	\item \textrm{Phase transition}:~相变
	\item \textrm{2D}本征磁性体系的拓扑性
\end{itemize}

\section{王先生点评}
\begin{itemize}
	\item 半导体的\textrm{dopping}
	\item \textrm{Fermi}电子态的问题
	\item \textit{d}电子的问题:~杂化和对称性
	\item 周期表和元素的选择
	\item 多尺度与陶瓷
	\item 复杂结构(多元素、多相、位错)
\end{itemize}
...
\section{课题分专项介绍}
\subsection{能源、环境材料的理论设计、筛选、性质和性能表征}
\begin{itemize}
	\item 电化学能源存储
	\item 电化学能量转换
	\item 光-能转化
...
过渡金属-轻金属的结合:~混合\textrm{~vs~}化合 (单一相(化合物)~\textrm{or}~混合物)\\
非金属基底，过渡金属\textrm{dopping}的两种情况
%\begin{equation} 
%\begin{split}
