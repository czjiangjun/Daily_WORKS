...
\section{研究背景}
催化研究，多学科交叉，计算化学的主要任务

\section{提问}
\begin{itemize}
	\item 催化剂的可重复利用:~瓶颈
	\item \ch{Fe-C}催化剂的颗粒:~\textrm{nm}到颗粒$\mathrm{\mu m}$
	\item 融铁催化剂\textrm{vm}喷雾催化剂
	\item 用穆谱定结构而没有用\textrm{XRD}谱:~\textrm{XRD}谱:~结构太近，没有辨析度;~核磁共振谱，磁性太强
	\item 催化结构:~铁的\textrm{d}电子中心，基本没有线性关系
	\item 非常复杂的耦合因素，有\textrm{HOMO/LUMO/\textit{d}}带中心的共同作用:~单个指标没有办法起作用
	\item 数据建设过程中，数据规范化，理论数据和实验数据的融合的经验:~数据整理的枯燥，数据的可靠性，反应器造型、结构、物料规范可控\\
		实验与理论的比对:~实验过程的数据带有太多的不确定因素，应该相信理论计算的最本征的数据\\
		思维理念的变革:~理论的数据要让实验的人理解
	\item 煤转油的经济效益和市场
	\item 结构搜索:~吉大最稳定结构搜索，这里重点是结合谱学的亚稳态结构搜索
	\item 结构搜索的缺陷的构建
\end{itemize}
